        ...
        public static List<Models.Modelo> Listar(int cliente_ID, VDT2.Models.Configuracao configuracao)
            ...
            try
                ...
                using (var contexto = new GeralDbContext(configuracao))
                    ...
                    var contador = listaModelo.Count();
                    #region gravalogResultado
                    Diag.Log.Grava(
                        new Diag.LogItem()
                            {
                            Nivel = Diag.Nivel.Informacao,
                            Mensagem = $"ModeloRepositorio.Listar realizado com sucesso:  Registros encontrados: {contador}"
                            });
                    #endregion  
                    return listaModelo;
                    }
                ...
                {
                listaModelo.Add(new Models.Modelo { Erro = true, MensagemErro = "Erro ao listar marcas DAL" });
                #region gravalogErro
                Diag.Log.Grava(
                    new Diag.LogItem()
                        {
